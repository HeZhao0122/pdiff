...
class ODVAE(nn.Module):
    ...
    def forward(self, input, fine_tune=False, **kwargs):
        x = self.encode(input)
    ...
    def freeze_encoder(self):
        ...
        for name, param in self.fc_mu.named_parameters():
            param.requires_grad = False
        for name, param in self.fc_var.named_parameters():
            param.requires_grad = False
...
class large(ODVAE):
    def __init__(self, in_dim, latent_dim, kld_weight):
        ...
        super(large, self).__init__(in_dim, latent_dim, kld_weight, kernel_size, fold_rate, enc_channel_list, dec_channel_list)

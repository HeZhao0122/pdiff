...
class ODEncoder2Decoder(nn.Module):
    def __init__(self, in_dim, kernel_size=3, fold_rate=3, input_noise_factor=0.001, latent_noise_factor=0.1, enc_channel_list=None, dec_channel_list=None):
        ...
        for i in range(len(dec_channel_list)):
            dim = real_input_dim // fold_rate ** (enc_layer_num - i)
            dec_dim_list.append(dim)

        self.real_input_dim = real_input_dim
        self.latent_dim = dec_dim_list[0]
        # self.pos_enc = nn.Linear(self.in_dim, 64)
        self.pos_enc = ODEncoder(enc_dim_list, fold_rate, kernel_size, enc_channel_list)
    ...
    def encode(self, x, **kwargs):
        ...
        # pos = self.pos_enc(pos).view(shape)
        # return self.encoder(x, **kwargs) + self.pos_enc(pos)
    ...
    def forward(self, input, **kwargs):
        x = self.add_noise(input, self.input_noise_factor)
        latent = self.encode(x)
        x = self.add_noise(latent, self.latent_noise_factor)
        x = torch.clamp(x, -1, 1)
        x = self.decode(x)
        return self.loss(x, input)


class ss(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 3
        kernel_size = 3
        enc_channel_list = [2, 2, 2]
        dec_channel_list = [2, 64, 8]
        super(ss, self).__init__(in_dim, kernel_size, fold_rate, input_noise_factor, latent_noise_factor, enc_channel_list, dec_channel_list)
...
class large(ODEncoder2Decoder):
    def __init__(self, in_dim, input_noise_factor, latent_noise_factor):
        fold_rate = 5
        kernel_size = 5
        enc_channel_list = [8, 8, 8, 8, 8]
        dec_channel_list = [8, 512, 1024, 512, 8]

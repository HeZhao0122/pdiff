...
from DWVae.modules.modules import Encoder, Decoder
# from modules.vqvae_module import VectorQuantizer, VectorQuantizerEMA, VqDecoder, VqEncoder
from DWVae.modules.distributions import DiagonalGaussianDistribution
...
class AutoencoderKL(nn.Module):
    ...
    def forward(self, input, sample_posterior=True):
        ...
        z, posterior = self.encode(input, True)
        # if sample_posterior:
        #     z = posterior.sample()
        # else:
        #     z = posterior.mode()
        dec = self.decode(z)
        mse = F.mse_loss(input, dec)
        aeloss, log_dict_ae = self.loss(input, dec, posterior, split="train")

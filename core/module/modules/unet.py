...
class AE_CNN_bottleneck(nn.Module):
    def __init__(
        ...
        # self.out_layer = nn.Conv1d(in_channel, self.org_dim, self.kernal_size, stride=1, padding=1)
    ...
    def forward(self, input, time, cond=None):
        ...
        # out = self.out_layer(emb_dec4).reshape(input_shape)
        return emb_dec4


class AE_CNN_bottleneck_small(nn.Module):
    def __init__(
            self,
            in_dim,
            in_channel,
        ...
        self.channel_list = [64, 128, 256]  # todo: self.channel_list*2
    ...
    def forward(self, input, time, cond=None):
        ...
        if self.self_cond:
            ...
            x_self_cond = x_self_cond.reshape(input.shape[0], 1, -1)
        ...
        emb_dec1 = self.dec1(emb_enc3 + time_info) + emb_enc2
        emb_dec2 = self.dec2(emb_dec1 + time_info) + emb_enc1
        emb_dec3 = self.dec3(emb_dec2 + time_info)
        # import pdb; pdb.set_trace()

        # emb_dec4 = emb_dec4.reshape(input_shape)
        # if self.dec is not None:
        #     emb_dec4 = self.dec.Dec(emb_dec4)
        # out = self.out_layer(emb_dec3).reshape(input_shape)
        out = emb_dec3.reshape(input_shape)
        return out


class AE_CNN_bottleneck_ss(nn.Module):
    def __init__(
            self,
            in_dim,
            in_channel,
        ...
        self.channel_list = [64, 128]  # todo: self.channel_list*2
    ...
    def forward(self, input, time, cond=None):
        ...
        # import pdb; pdb.set_trace()
        emb_dec1 = self.dec1(emb_enc2 + time_info) + emb_enc1
        emb_dec2 = self.dec2(emb_dec1 + time_info)
        # import pdb; pdb.set_trace()
        ...
        #     emb_dec4 = self.dec.Dec(emb_dec4)
        # out = self.out_layer(emb_dec3).reshape(input_shape)
        out = emb_dec2.reshape(input_shape)
        return out
...
class AE_small(nn.Module):
    def __init__(
        ...
        self.time_encode = nn.Embedding(time_step, self.real_input_dim)
        self.cond_emb = nn.Sequential(nn.Linear(cond_dim, self.in_dim),nn.Tanh())
        # self.latent_enc = torch.load('./param_data/param_encoder.pt')
    ...
    def forward(self, input, time, cond=None):
        ...
        # cond_emb = cond_emb.reshape(input_shape[0], -1, input_shape[2])
        # input = input + cond_emb

        # input = self.latent_enc(input.reshape(input_shape[0], -1))

        input = input.reshape(input.shape[0], 1, -1)
        input = input + cond.unsqueeze(1)
        # input = torch.cat([input, self.cond_emb(cond).unsqueeze(1)], dim=-1)
        ...
        # emb_dec3 = self.out(emb_dec3)

        emb_dec3 = emb_dec3.reshape(input_shape)
        # if self.dec is not None:
        #     emb_dec4 = self.dec.Dec(emb_dec4)
...
class TF(nn.Module):
    def __init__(
        ...
        self.cond_emb = nn.Linear(cond_dim, self.len_token)

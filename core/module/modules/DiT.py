...
class OnedPatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B,C,D = x.shape
        assert D == self.img_size, \
            f"Input image size ({D}) doesn't match model ({self.img_size})."
        x = self.proj(x)
        if self.flatten:
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.x_embedder = OnedPatchEmbed(input_size, patch_size, in_channels, hidden_size)
        ...
        self.shape_embedder = nn.Sequential(nn.Linear(3, hidden_size), nn.Tanh())
...
def get_1d_sincos_pos_embed(embed_dim, grid_size, cls_token=False, extra_tokens=0):
    ...
    grid_size: int of the grid height and width
    return:
    pos_embed: [grid_size, embed_dim] or [1+grid_size*grid_size, embed_dim] (w/ or w/o cls_token)
    """
    grid_h = np.arange(grid_size, dtype=np.float32)
    # grid = np.meshgrid(1, grid_h)  # here w goes first
    # grid = np.stack(grid_h, axis=0)

    grid = grid_h.reshape([1, 1, grid_size])

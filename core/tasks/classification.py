...
from core.data.Graph_dataset import GraphCFTData
...
class CFTask(BaseTask):
    ...
    def train_for_data(self, seed=1):
        ...
        final_path = os.path.join(final_path, 'all_seed/' + str(seed))

        os.makedirs(tmp_path, exist_ok=True)
        os.makedirs(final_path, exist_ok=True)

        if self.cfg.noise_test:
            net = torch.load(os.path.join(final_path, "data.pt"))['model'].cuda()
            # batch = torch.load(os.path.join(final_path, "data.pt"))['pdata'].cuda()
            params = [param.data.reshape(-1) for name, param in net.named_parameters() if name in train_layer]
            params = torch.cat(params)
            print(f'param size: {params.shape}')
            test_acc = self.test(net, criterion, self.test_loader)
            # n_acc, n_test = [], []
            acc, test_l = self.noise_test(params, net)
            # n_acc.append(acc)
            # n_test.append(test_l)
            # print(f'noised acc: {acc}')
            # # print(f'noised mean acc: {sum(n_acc) / len(n_acc)}')
            # print(f'noised test loss: {test_l}')
            # print(f'performance decrease: {test_acc - acc}')
        else:

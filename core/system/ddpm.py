...
class DDPM(BaseSystem):
    def __init__(self, config, **kwargs):
        ...
        self.self_condition = self.config.self_condition
        self.threshold = self.config.p
    ...
    def betas_register(self, betas):
        ...
        self.register("alphas_cumprod", alphas_cumprod)  # \hat{alpha_t}
        self.register("alphas_cumprod_prev", alphas_cumprod_prev)   # \hat{alpha_{t-1}}
    ...
    def progressive_samples_fn_simple(self, model, shape, device, include_x0_pred_freq=50, history=False, initial=None, cond=None):
        samples, history = self.p_sample_loop_progressive_simple(
            ...
            cond=cond,
            input_pa=None if initial is None else initial,
        )
        if history:
    ...
    def test_step(self, pbatch, batch_idx, **kwargs: Any):
        # generate 100 models
        # initial = torch.load('./data/CiteSeer/first_param.pt')
        # initial = initial.cuda().view(1,-1)
        # # transfer
        # i_batch = self.pre_process(initial)
        # i_outputs = self.generate(i_batch, 100, initial=i_batch)
        # i_params = self.post_process(i_outputs)
        # path = './param_data/PubMed/generate.pt'
        # torch.save(i_params.cpu(), path)
        cond = pbatch[2]
        batch = pbatch[0]
        labels = pbatch[3]
        mask = pbatch[1].float()
    ...
    def forward(self, pbatch, **kwargs):
        ...
        # if self.self_condition and random() > self.threshold:
        #     with torch.no_grad():
        #         _, x_self_cond = self.p_sample(model, x_t, time, torch.randn, return_pred_x0=True)
        #         lab = x_self_cond.detach_()
                # lab = x_t.detach_()
        ...
        self.log('ddpm_train_loss', loss, on_epoch=True, prog_bar=True, logger=True)
        return loss
    ...
    def p_sample_loop_progressive_simple(self, model, shape, device, noise_fn=torch.randn,
                                         include_x0_pred_freq=50,cond=None,input_pa=None,exp_step=None):

...
class BaseSystem(pl.LightningModule, abc.ABC):
    ...
    def task_func(self, input, hidden_dim=None):
        return self.task.test_g_model(input, hidden_dim)

    def task_val_func(self, input, hidden_dim=None):
        return self.task.val_g_model(input, hidden_dim)
